...
PRO plot_sensitivity1d, trigger=trigger, sensitivity, cutoff, label=label, title=title, yrange=yrange, noeps=noeps
	if ~keyword_set(yrange) then yrange = [1,100]
 if ~keyword_set(noeps) then set_plot, 'ps'
  common mearth_tools
...
; restore, f
cleanplot
 if keyword_set(noeps) then xplot
  !p.charthick=3
...
if ~keyword_set(noeps) then  device, filename=label+'sensitivity.eps', /encapsulated, /color, /inches, xsize=10, ysize=5
  !p.charsize=1.1
!x.charsize=1.2
!y.charsize=1.2
...
;         al_legend, box=0, thick=5, color=radii_color, goodtex(string(format='(F4.1)', radii) + ' R_{Earth}'), /top, /right, linestyle=0;subset
  ...
  smultiplot
  plot, sensitivity.period.grid, sensitivity.period.detection[*,0], /xstyle, /ystyle, xrange=[0.6,20], xtitle='Period (days)', ytitle=goodtex('MEarth Sensitivity !C!D(effective # of stars searched,!C!Dincluding transit probability!C!D+ per-star sensitivity)'), /ylog, /xlog, yrange=yrange, /nodata, title=title; title='                      '+label
...
smultiplot
  plot, sensitivity.temp.grid, sensitivity.temp.detection[*,0], /xstyle, /ystyle, xtitle='Equilibrium Temperature (K)!C!D[assuming zero albedo]', /ylog, yrange=yrange, xrange=[250,max(sensitivity.temp.grid)], /nodata;, title=label
...
print, angle
 ...
 ;   xyouts, [300,350, 400], [8.5,4.5,3], goodtex(string(format='(F3.1)', radii[subset]) + ' R_{Earth}'), alignment=0.5, orientation=50
  ...
  smultiplot, /def
 if ~keyword_set(noeps) then device, /close

  if ~keyword_set(noeps) then epstopdf,label+'sensitivity.eps'

;estimate_occurrence, /plot, sensitivity_filename=filename
  set_plot, 'x'
