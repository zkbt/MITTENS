PRO marpleplot_rms, eps=eps, n_sigma, te=te, oneyear=oneyear
	@planet_constants
	cleanplot
	if ~ keyword_set(n_sigma) then n_sigma = 3


	filename = 'marpleplot_rms.eps'
	if keyword_set(oneyear) then filename = 'oneyear_marpleplot_rms.eps'
...
; 	endif
	...
	for i=start, n_elements(years)-1 do begin
		smultiplot
		p = load_photometric_summaries(ye=years[i], n=100, te=te, /unknown)
		if i eq start then ytitle=goodtex(rw(n_sigma) + '\sigma Per-Point Photometric Uncertainty!CBefore Any Cleaning  (Earth radii)') else ytitle=''
