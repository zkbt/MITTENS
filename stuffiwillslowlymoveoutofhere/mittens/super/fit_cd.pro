FUNCTION r_disc_model, radius, period, coefs=coefs
...
FUNCTION negativeloglikelihood, params
	common fitting_vars, inputs, plot
	...
	r_fine = r_disc_model(inputs.radius_grid, inputs.period_grid, coef=inputs.disc_coef)
	...
	if keyword_set(plot) then begin
		...
		xlog = 1
		ylog = 1
		r = r_disc_model(KOI.rp, KOI.period, coef=inputs.disc_coef)
		eta = r/(1.0+r)
...
seed = systime(/sec)
	...
	;	KOI = KOI[where(KOI.rp lt 4.5 and KOI.rp gt 0.8)]
		...
		r = koi.r_disc;r_disc_model(KOI.rp, KOI.period, coefs=coefs)
		eta = r/(1.0+r)
		symsize = sqrt(KOI.a_over_rs/eta)
		symsize = symsize/max(symsize)*10
		theta = findgen(21)/!pi
		...
		plot, /nodata, KOI.period, KOI.rp, yr=[0.5, 4], /xlog, ys=3
		r = r_disc_model(KOI.rp, KOI.period, coef=disc_coef)
		eta = r/(1.0+r)
		symsize = sqrt(KOI.a_over_rs/eta)
...
;		fit = amoeba(0.001, function_name="negativeloglikelihood", p0=params, scale=abs(params)/100.0)
;;;;;;;;;;;;;;;;;;;;;;
	...
	for i=0l, n-1 do begin
		...
		proposed_loglikelihood = -negativeloglikelihood(struct_to_array(proposed))
	...
	save, chain, loglikelihood, was_accepted, filename='fitcd_trimmedsuperneptune_'+prefix+'_chain.idl'
