...
;   	lspm_string = stregex(/ex, star_dir, 'ls[0-9]+')
		;b = big_bls[0, which]
		openw, email_lun, /get_lun, star_dir + 'email.txt'

	restore, star_dir + 'inflated_lc.idl'
		i_intransit = where_intransit(inflated_lc, candidate, n_intransit)
		if n_intransit gt 0 then begin
			transit_number = round((inflated_lc[i_intransit].hjd - candidate.hjd0)/candidate.period)
	...
	printf, email_lun, 'CURRENT BEST CANDIDATE'
				...
				printf, email_lun,  '   uncertainty = ', string(candidate.depth_uncertainty, format ='(D6.4)')
				printf, email_lun,  '   S/N = ', string(candidate.depth/candidate.depth_uncertainty, format ='(D5.1)')
				printf, email_lun,  '   number of boxes = ', string(candidate.n_boxes, format ='(I)')
				printf, email_lun,  '   number of points = ', string(candidate.n_points, format ='(I)')
...
printf, email_lun, ''
	...
	caldat, systime(/julian)-10, month, day, year
	printf, lun, string(format='(I4)', year) + ' ' + string(format='(I2)', month) + ' ' + string(format='(I2)', day)
	caldat, systime(/julian)+60, month, day, year
	printf, lun,  string(format='(I4)', year) + ' ' + string(format='(I2)', month) + ' ' + string(format='(I2)', day)
	printf,  lun, '-1'
	printf,  lun, '90'
	printf, lun, 'Q'
