PRO plot_iac80, events=events

	filename = '/pool/barney0/jirwin/mearth/lspm2986/iac80/reduced/lc.fits'
	m = mrdfits('/pool/barney0/jirwin/mearth/lspm2986/iac80/reduced/lc.fits', 1, h)
...
; 									; define comparison star lightcurves
								...
								star_dir = '/pool/eddie1/zberta/mearth_most_recent/ls2986/ye11/te11/'
								file_mkdir, star_dir
	...
	tagstoplot = ['LEFT_XLC', 'LEFT_YLC', 'PEAK', 'SEE', 'EXPTIME']
	labels       = ['X', 'Y', 'PEAK', 'SEEING', 'EXPTIME']


	set_plot, 'ps'
	device, filename='iac80_lc.eps', /encapsulated, /color, xsize=10, ysize=7, /inches
...
; 		candidate = best_candidates[i]
		...
		this_event = round((mean(hjd) - candidate.hjd0)/candidate.period)
...
;
	...
	device, /close
	epstopdf, 'iac80_lc.eps'
	target_lc[150:*].okay = 0B
