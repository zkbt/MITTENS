PRO mega_fold_boxes, remake=remake, n_epochstotry=n_epochstotry, p_min=p_min, p_max=p_max, profile=profile, highres=highres
;+
; NAME:
;	PHASE_PDF 
; PURPOSE:
...
; CALLING SEQUENCE:
;	phaseup, transit, n_lc=n_lc, n_epochstotry=n_epochstotry, eps=eps, folder=folder, candidate=candidate, fast=fast, lc=lc
; INPUTS:
;	transit = structure containing anchoring event
;	(knows about star directory through "this_star" common block)
; KEYWORD PARAMETERS:
;	n_lc=n_lc, n_epochstotry=n_epochstotry, eps=eps, folder=folder, candidate=candidate, fast=fast, lc=lc
; OUTPUTS:
; RESTRICTIONS:
; EXAMPLE:
...
;-
	...
	if keyword_set(profile) then begin
		...
		profiler, /system
		profiler
		mega_fold_boxes, remake=remake, n_epochstotry=n_epochstotry, p_min=p_min, p_max=p_max
...
;		if question('do you want to look at the profiler results?', /int) then stop
	...
	if is_uptodate(star_dir + 'mega_candidates_pdf.idl', star_dir + 'box_pdf.idl') and not keyword_set(remake) then begin
...
; 		plot_boxes, boxes
	...
	if not keyword_set(n_epochstotry) then n_epochstotry = 25
	...
	i_startingbox = intarr(n_depths, n_epochstotry)
	for i=0, n_depths-1 do begin
		...
		if n_elements(sorted) ge n_epochstotry then i_startingbox[i,*] = i_interesting[sorted[0:n_epochstotry-1]] else i_startingbox[i,0:n_elements(sn)] = i_interesting[sorted]

		if keyword_set(display) then begin
			plot, sn[sorted], xr=[0,2*n_epochstotry]
			vline, n_epochstotry
		endif
;stop
; 		; set the starting boxes for this duration to the peaks of that S/N spectrum
; 		i_startingbox[i,*] = i_interesting[select_peaks(sn, n_epochstotry, n_sigma=1)]
	endfor
...
;	v_bin = 5.0d/60.0/24.0/(max(lc.hjd) - min(lc.hjd))
	...
	maxdurations = periods/a_over_r/4.0
 ...
 n_save = 10
	...
	;candidates =;replicate({period:0.0d, hjd0:0.0d, duration:0.0, depth:0.0, depth_uncertainty:0.0, n_boxes:0, n_points:0, rescaling:1.0}, n_periods, n_epochstotry, n_durations) 
 	peak_candidates = replicate({period:0.0d, hjd0:0.0d, duration:0.0, depth:0.0, depth_uncertainty:0.0, n_boxes:0, n_points:0, rescaling:1.0}, n_epochstotry, n_durations, n_save) 
	...
	for j=0, n_epochstotry-1 do begin
		
		; loop over durations (effectively impact parameter)
		for k=0, n_durations-1 do begin
			; skip out on the super lame ones
			if boxes[i_startingbox[k, j]].depth[k]/boxes[i_startingbox[k, j]].depth_uncertainty[k] lt 0 then continue
			...
			for i=0L, n_periods-1 do begin
				...
				if not keyword_set(interactive) and i mod 1000 eq 0 then counter, i, n_periods, /timeleft, starttime=starttime, $
					...
					' epoch ' + rw(j+1) + ' of ' + rw(n_epochstotry) + ' | ' +$
					' period '	
			endfor
			...
			if total(finite(sn)) gt 0 then begin
				...
				if keyword_set(display) then begin
					...
					!p.color=150
					oploterr, 24*phased_time[i_interesting], boxes[i_interesting].depth[k_best], boxes[i_interesting].depth_uncertainty[k_best], 8
					!p.color=0

					hline, 0, linestyle=1
					vline, -best_candidate.duration/2*24, linestyle=2
					vline, best_candidate.duration/2*24, linestyle=2
					oploterr, 24*phased_time[i_intransit[uniq_intransit]], boxes[i_intransit[uniq_intransit]].depth[k_best], boxes[i_intransit[uniq_intransit]].depth_uncertainty[k_best],  8
					loadct, 39, /silent
					!p.color=250
					oploterr, best_candidate.depth, best_candidate.depth_uncertainty, 3
					!p.color=0

					smultiplot, /dox
					loadct, 0, /silent
					plot, 24*phased_time[i_interesting], boxes[i_interesting].depth[k_best],xr=24*[-best_candidate.duration, best_candidate.duration]*3, psym=8, yrange=yr, xtitle='Phased Time (hours)',/nodata, xmargin=[30,5]
					!p.color=150
					oploterr, 24*phased_time[i_interesting], boxes[i_interesting].depth[k_best], boxes[i_interesting].depth_uncertainty[k_best], 8
					!p.color=0
					hline, 0, linestyle=1
		...
		; save results
		save, filename=star_dir + 'mega_candidates_pdf.idl', best_candidates
		n_boxes = n_elements(boxes)
		n_datapoints = n_elements(lc)
		save, filename=star_dir + 'mega_spectrum_pdf.idl' , p_max, p_min, n_periods, n_durations, n_boxes, n_datapoints, max_misalign, data_span, squashed_sn
	endfor
