...
common xlc_time_common, xlc_time_camera, xlc_time_coordinate_conversions
...
;*** appropriate action.
  "draw": begin
			...
			if xlc_time_camera.setting_range_right then begin
				if event.press eq 1 then begin
					...
					widget_control, xlc_time_camera.messageid, set_val='Zooming in to ' + string(form='(F5.1)', xlc_time_camera.xrange[0]) + ' to ' + string(form='(F5.1)', xlc_time_camera.xrange[1])
				endif	
				if event.release eq 1 then begin
					xlc_time_camera.setting_range_right = 0
					widget_control, xlc_time_camera.messageid, set_val=''
				endif
			endif
			...
			if xlc_time_camera.setting_range_left then begin
				...
				if event.release eq 1 then begin
					widget_control, xlc_time_camera.messageid, set_val='Please pick the RIGHT side of your zoom range.'
 ...
 "zoom - ": 	begin
				...
				if  xlc_time_camera.xrange[0] ne 0 or xlc_time_camera.xrange[1] ne 0 then begin
					center = mean(xlc_time_camera.xrange)
					span = max(xlc_time_camera.xrange)- min(xlc_time_camera.xrange)
					xlc_time_camera.xrange = center + [-1.,1.]*span/2.0*2.0
  ...
  "zoomselect": 	begin
					widget_control, xlc_time_camera.messageid, set_val='Please pick the LEFT side of your zoom range.'
  ...
  "optiondiagnostics" : xlc_time_camera.diagnostics = ~xlc_time_camera.diagnostics
...
rightborder = 100
leftborder = 130
xlc_timebase = WIDGET_BASE(TITLE = "xlc_time", xsize=width + rightborder + leftborder, ysize=height + topborder + bottomborder)	;create the main base

id = xlc_timebase
;*** Here some default controls are built in a menu.  The descriptions of these
;*** procedures can be found in the xlc_time_ev routine above.  If you would
;*** like to add other routines or remove any of these, remove them both below
;*** and in the xlc_time_ev routine.

toprow =  WIDGET_BASE(xlc_timebase, /row, xoffset=leftborder,/base_align_center)
middlerow =  WIDGET_BASE(xlc_timebase, col=3, ysize=height,xoffset=0, yoffset=topborder,/base_align_center)
bottomrow =  WIDGET_BASE(xlc_timebase, /column,  ysize=bottomborder,xoffset=leftborder, xsize=width,yoffset=topborder + height, /base_align_center)
...
;WIDGET_CONTROL, xlc_timebase, /REALIZE			;create the widgets

; geo = widget_info(xlc_timebase, /geometry)
...
;*** Create them and use xlc_timebase as their base.  They will be realized
...
xlc_time_camera = {zoom:zoom, shift:shift, scale:scale, binned:0, model:1, raw:0, outliers:1, intransit:0, histograms:1, diagnostics:0, comparisons:0, anonymous:0, eps:0, png:0, basic:1, variability:1, cleaned:1, symsize:0.6, original_scale:scale, drawid:draw, messageid:message_box, setting_range_left:0, setting_range_right:0, xrange:[0.,0.]}


XManager, "xlc_time", xlc_timebase, $			;register the widgets
		EVENT_HANDLER = "xlc_time_ev", $	;with the XManager
...
END ;==================== end of xlc_time main routine =======================
