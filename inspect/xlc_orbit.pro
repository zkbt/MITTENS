...
PRO xlc_orbit_ev, event
	...
	common xlc_orbit_common, xlc_orbit_camera, xlc_orbit_coordinate_conversions, xlc_orbit_select, xlc_orbit_selected_times, xlc_orbit_ds9, xlc_orbit_censored_exposures, xlc_orbit_censored_filenames, xlc_orbit_censorship
	@widget_geometries
	common this_star
	common mearth_tools
	...
	CASE thebuttonclicked OF
		...
		"draw": begin
			...
			data_click = smulti_datacoord(event=event, coordinate_conversions=xlc_orbit_coordinate_conversions, geometry=geometry)
			...
			if event.press eq 2 then begin
				if ~xlc_orbit_camera.setting_zoom_left and ~xlc_orbit_camera.setting_zoom_right then xlc_orbit_camera.setting_zoom_left = 1
			endif
			...
			if xlc_orbit_camera.setting_zoom_right then begin
				...
				if event.press gt 0 then begin
					...
					widget_control, xlc_orbit_camera.message_label_id, set_val='Zooming in to ' + string(form='(F5.1)', xlc_orbit_camera.xrange[0]) + ' to ' + string(form='(F5.1)', xlc_orbit_camera.xrange[1])
					; prevent freaking out if selected xrange is reverse
					if xlc_orbit_camera.xrange[0] gt xlc_orbit_camera.xrange[1] then begin
						xlc_orbit_camera.xrange = reverse(xlc_orbit_camera.xrange)
					endif
				endif	
				; when the mouse button is released
				if event.release gt 0 then begin
					xlc_orbit_camera.setting_zoom_right = 0
					widget_control, xlc_orbit_camera.message_label_id, set_val=''
				endif
			endif

			; if we're starting to set an xrange zoom range, record the position of the click
			if xlc_orbit_camera.setting_zoom_left then begin
				...
				if event.release gt 0 then begin
					...
					widget_control, xlc_orbit_camera.message_label_id, set_val='Please pick the RIGHT side of your zoom range.'
					; switch to picking the other side of the zoom range
					xlc_orbit_camera.setting_zoom_left = 0
					xlc_orbit_camera.setting_zoom_right = 1
			...
			if xlc_orbit_camera.setting_select_right then begin
				; when the mouse button is pushed
				if event.press gt 0 then begin
					...
					; set the right side of the xrange to the mouse click
					xlc_orbit_camera.select_xrange[1] = data_click.x
					xlc_orbit_camera.select_yrange[1] = data_click.y

					oplot_box, xlc_orbit_camera.select_xrange, xlc_orbit_camera.select_yrange, thick=3
					print,  xlc_orbit_camera.select_xrange, xlc_orbit_camera.select_yrange
; 					; say that we're selecting in
; 					widget_control, xlc_orbit_camera.message_label_id, set_val='selecting in to ' + string(form='(F5.1)', xlc_orbit_camera.xrange[0]) + ' to ' + string(form='(F5.1)', xlc_orbit_camera.xrange[1])
					; prevent freaking out if selected xrange is reverse
					if xlc_orbit_camera.select_xrange[0] gt xlc_orbit_camera.select_xrange[1] then begin
						xlc_orbit_camera.select_xrange = reverse(xlc_orbit_camera.select_xrange)
					endif
					if xlc_orbit_camera.select_yrange[0] gt xlc_orbit_camera.select_yrange[1] then begin
						xlc_orbit_camera.select_yrange = reverse(xlc_orbit_camera.select_yrange)
					endif
				endif	
				; when the mouse button is released
				if event.release gt 0 then begin
					xlc_orbit_camera.setting_select_right = 0
					widget_control, xlc_orbit_camera.message_label_id, set_val=''
				endif
			endif

			; if we're starting to set an xrange select range, record the position of the click
			if xlc_orbit_camera.setting_select_left then begin
				; when the button is pressed
				if event.press gt 0 then begin
					; draw crosshair
					plots, data_click.x, data_click.y, psym=1, symsize=2, thick=3
					...
					plots, /normal, xbox, ybox, thick=4
					; record the mouse click position
					xlc_orbit_camera.select_xrange[0] = data_click.x
					xlc_orbit_camera.select_yrange[0] = data_click.y
					xlc_orbit_camera.select_which = data_click.which
				endif
				; when the button is released
				if event.release gt 0 then begin
					; ask the user to set the rest of the select range
					widget_control, xlc_orbit_camera.message_label_id, set_val='Click UPPER RIGHT of points to select.'
					; switch to picking the other side of the select range
					xlc_orbit_camera.setting_select_left = 0
					xlc_orbit_camera.setting_select_right = 1
		...
		"zoom - ": begin
			...
			if  xlc_orbit_camera.xrange[0] ne 0 or xlc_orbit_camera.xrange[1] ne 0 then begin
				center = mean(xlc_orbit_camera.xrange)
				span = max(xlc_orbit_camera.xrange)- min(xlc_orbit_camera.xrange)
				xlc_orbit_camera.xrange = center + [-1.d,1.d]*span/2.0d*2.0d
			endif
		end
		...
		"zoomcustom": 	begin
			widget_control, xlc_orbit_camera.message_label_id, set_val='Please pick the LEFT side of your zoom range.'
			xlc_orbit_camera.setting_zoom_left = 1
		end
		; to input a specific MJD and go to that spot
		"mjd":begin
			widget_control, xlc_orbit_camera.mjdbox_id, get_value=userinputmjd
			center = double(userinputmjd[0])
			restore, star_dir() + 'cleaned_lc.idl'
			pad = 100000000.0d
			phased_time = (center-candidate.hjd0)/candidate.period + pad + 0.5
				orbit_number = long(phased_time)
				center = (phased_time - orbit_number - 0.5)*candidate.period*24
				xlc_orbit_camera.xrange = center+ [-0.5d, 0.5d]*candidate.duration*5
			widget_control, xlc_orbit_camera.mjdbox_id, set_value=''

		end
		; to input a specific "night" in YYYYMMDD format and go to that spot
		"night":begin
			widget_control, xlc_orbit_camera.nightbox_id, get_value=userinputnight
			...
			widget_control, xlc_orbit_camera.nightbox_id, set_value=''
		end
		...
		"optiondiagnostics" : begin
			xlc_orbit_camera.diagnostics = ~xlc_orbit_camera.diagnostics
			if xlc_orbit_camera.diagnostics then begin
				new_ysize = height*2 + xlc_orbit_camera.ypad
			endif else begin
				new_ysize = height + xlc_orbit_camera.ypad
			endelse
			resized = 1	
		end
		...
		"eps" : begin
			widget_control, xlc_orbit_camera.message_label_id, set_val='Saving to EPS; acroread will open shortly.'
			xlc_orbit_camera.eps=1
		end
		; to save the current plot to png		
		"png" : begin
			widget_control, xlc_orbit_camera.message_label_id, set_val='Saving to PNG; konqueror will open shortly.'
			xlc_orbit_camera.png=1
		end
		...
		; to use the mouse to select the zoom range (can also click on the plot with middle mouse buttons)
		"select": begin
			widget_control, xlc_orbit_camera.message_label_id, set_val='Click to the LOWER LEFT of points to select.'
			xlc_orbit_camera.setting_select_left = 1
		end
		"unselect": begin
			xlc_orbit_camera.setting_select_left = 0
			xlc_orbit_camera.setting_select_right = 0
			xlc_orbit_camera.select_xrange = [0,0]
			xlc_orbit_camera.select_yrange = [0,0]
		end
		'ds9':begin
			ds9_some_hjds, xlc_orbit_selected_times.raw, xpa_name=xpa_name,  filenames=filenames, pid=pid
			xlc_orbit_ds9 = {xpa_name:xpa_name, filenames:filenames, hjds:xlc_orbit_selected_times.raw, pid:pid};, flagged:bytarr(n_elements(xlc_orbit_selected_times.raw))}
			print, xpa_name
		end
		'flag image':begin
			if n_tags(xlc_orbit_ds9) gt 0 then begin
				flag_ds9, xpa_name=xlc_orbit_ds9.xpa_name, image_filenames=xlc_orbit_ds9.filenames, image_hjds=xlc_orbit_ds9.hjds, censored_exposures=xlc_orbit_censored_exposures, censored_filenames=xlc_orbit_censored_filenames
			endif
		end
		'censor': begin
			xlc_orbit_camera.perform_censoring = 1
		end	
		'save':begin
			n_flagged_in_ds9 =  n_elements(xlc_orbit_censored_exposures) 
			if n_flagged_in_ds9 gt 0 then begin
				openw, raw_censor_lun, /get_lun, star_dir + 'raw_image_xlc_orbit_censorship.log', /append
				for i=0, n_flagged_in_ds9-1 do begin
					if total(xlc_orbit_censored_exposures eq xlc_orbit_censored_exposures[i]) mod 2 ne 0 then begin
						printf, raw_censor_lun, string(format='(D13.7)', xlc_orbit_censored_exposures[i]) + ' = ' + xlc_orbit_censored_filenames[i] + ' was censored by '+username+' with ds9 on ' + systime()
					endif
				endfor
				close, raw_censor_lun
				spawn, 'kwrite ' + star_dir + 'raw_image_xlc_orbit_censorship.log'
				spawn, 'touch ' + star_dir + 'needtomakemarple'
			endif 
			if n_tags(xlc_orbit_ds9) gt 0 then begin	
				; quit the ds9 window you were using
				spawn, 'xpaset -p '+ xlc_orbit_ds9.xpa_name +' quit;'
			endif
			if n_tags(xlc_orbit_censorship) gt 0 then begin
				if total(xlc_orbit_censorship.okay, /int) ne n_elements(xlc_orbit_censorship) then begin
					openw, censor_lun, /get_lun, star_dir + 'xlc_orbit_censorship.log', /append
					i_censor=where(xlc_orbit_censorship.okay eq 0, n_censor)
					for i=0, n_censor-1 do printf, censor_lun, string(format='(D13.7)', xlc_orbit_censorship[i_censor[i]].hjd) + ' was censored by hand by '+username+' on ' + systime()
					close, censor_lun
					spawn, 'kwrite ' + star_dir + 'xlc_orbit_censorship.log'
					spawn, 'touch ' + star_dir + 'needtomakemarple'
				endif
			endif
...
; 	
	...
	if keyword_set(resized) then begin
		...
		widget_control, xlc_orbit_camera.base_id, xsize=new_xsize, ysize=new_ysize

		; figure out how big the window is
		frame_info =  widget_info(xlc_orbit_camera.base_id, /geom)
		frame_height = frame_info.ysize
		frame_width = frame_info.xsize
		draw_width = frame_width - xlc_orbit_camera.xpad
		draw_height = frame_height -  xlc_orbit_camera.ypad;(topborder + bottomborder )
	
		; resize the draw window
		widget_control, xlc_orbit_camera.draw_id, xsize=long(draw_width), YSIZE =long(draw_height)
		wset, xlc_orbit_camera.draw_window
		erase
		print, 'draw'
;		help, widget_info(xlc_orbit_camera.draw_id, /geom)
		printl
	endif
...
;	print, 'xrange is ', xlc_orbit_camera.xrange
	
	needtoplot=1B
	while (needtoplot $
		and xlc_orbit_camera.setting_zoom_left eq 0 and xlc_orbit_camera.setting_zoom_right eq 0$
		and xlc_orbit_camera.setting_select_left eq 0 and xlc_orbit_camera.setting_select_right eq 0) do begin
		set_plot, 'x'
		wset, xlc_orbit_camera.draw_window
		device, decomposed=0
		cleanplot, /silent
		...
		lc_plot, /phased,  zoom=xlc_orbit_camera.zoom, shift=xlc_orbit_camera.shift, scale=xlc_orbit_camera.scale, /externalformat, charsize=1, symsize=xlc_orbit_camera.symsize, /time,binned=xlc_orbit_camera.binned, anonymous=xlc_orbit_camera.anonymous, no_model=~keyword_set(xlc_orbit_camera.model), no_cleaned=~keyword_set(xlc_orbit_camera.cleaned), no_var=~keyword_set(xlc_orbit_camera.variability), no_raw=~keyword_set(xlc_orbit_camera.raw), no_outliers=~keyword_set(xlc_orbit_camera.outliers), noright=~keyword_set(xlc_orbit_camera.histograms), no_intransit=~keyword_set(xlc_orbit_camera.intransit), eps=keyword_set(xlc_orbit_camera.eps), png=keyword_set(xlc_orbit_camera.png), diagnos=xlc_orbit_camera.diagnostics, compa=xlc_orbit_camera.comparisons, coordinate_conversions=coordinate_conversions, xrange=xrange, select_xrange=xlc_orbit_camera.select_xrange, select_yrange=xlc_orbit_camera.select_yrange , select_which= xlc_orbit_camera.select_which, selected_times = xlc_orbit_selected_times, censorship=xlc_orbit_censorship, perform_censoring=xlc_orbit_camera.perform_censoring
	
		xlc_orbit_camera.perform_censoring = 0
		needtoplot =0B
...
;	help, xlc_orbit_selected_times
	if n_tags(xlc_orbit_selected_times) gt 0 then begin
		widget_control, xlc_orbit_camera.select_label_id, set_value=rw(xlc_orbit_selected_times.n_raw) + ' exp. + ' + rw(xlc_orbit_selected_times.n_binned) + ' obs.'+ 'selected'
		widget_control, xlc_orbit_camera.ds9_button_id, sensitive=xlc_orbit_selected_times.n_raw gt 0 or xlc_orbit_selected_times.n_binned gt 0 
		widget_control, xlc_orbit_camera.flag_ds9_button_id, sensitive=xlc_orbit_selected_times.n_raw gt 0 or xlc_orbit_selected_times.n_binned gt 0 
		widget_control, xlc_orbit_camera.censor_button_id, sensitive=xlc_orbit_selected_times.n_raw gt 0 or xlc_orbit_selected_times.n_binned gt 0 
	endif else begin
		widget_control, xlc_orbit_camera.ds9_button_id, sensitive=0
		widget_control, xlc_orbit_camera.flag_ds9_button_id, sensitive=0
		widget_control, xlc_orbit_camera.censor_button_id, sensitive=0
	endelse
...
PRO xlc_orbit, GROUP = GROUP, BLOCK=block, id=id
	...
	; only run one xlc_orbit widget at a tiem
	IF(XRegistered("xlc_orbit") NE 0) THEN begin
	RETURN	
	endif							
	...
	; make the main xlc_orbit window
	xlc_orbitbase = WIDGET_BASE(TITLE = "xlc_orbit", row=3,/base_align_center, /tlb_size_events);
	
	; define the window's ID so it can be killed externally
	id = xlc_orbitbase
	
	; set up three rows for widget base
	toprow_base =  WIDGET_BASE(xlc_orbitbase, /row, /base_align_center, frame=3);xoffset=leftborder/2,
		output_buttons_base = widget_base(toprow_base, /row, /align_center)
		messagebox_base = widget_base(toprow_base, /row, /align_center)
	middlerow_base =  WIDGET_BASE(xlc_orbitbase, /base_align_center, col=3, frame=3);xoffset=0, ysize=height, yoffset=topborder,/base_align_center
	bottomrow_base =  WIDGET_BASE(xlc_orbitbase, /row, /base_align_center, frame=3);xoffset=0, ysize=bottomborder,yoffset=topborder + height, 
...
;	context_button = widget_button(bottomselect_base, uvalue='context', value='see in context', sensitive=0)
	...
	WIDGET_CONTROL, xlc_orbitbase, /REALIZE
	...
	!p.color=0
	plot, [0], /nodata, xs=4, ys=4
	lc_plot, /phased,  zoom=zoom, shift=shift, scale=scale, /externalformat, charsize=1, symsize=0.6, /time, binned=0, /no_raw, /no_int, coordinate_conversions=coordinate_conversions, failed=failed
	if keyword_set(failed) then return
	if n_elements(coordinate_conversions) gt 0 then xlc_orbit_coordinate_conversions = coordinate_conversions
	


	; define a structure that contains variables needed to keep track of options
	xlc_orbit_camera = {	zoom:zoom, shift:shift, scale:scale, xrange:[0.d,0.d], $
						xpad:0, ypad:0, $
	...
	junk = temporary(xlc_orbit_select)
	junk = temporary(xlc_orbit_selected_times)
junk = temporary(xlc_orbit_ds9)
junk = temporary(xlc_orbit_censored_exposures)
junk = temporary( xlc_orbit_censored_filenames)
junk = temporary( xlc_orbit_censorship)
	xlc_orbit_censorship = 0

	frame_info =  widget_info(xlc_orbit_camera.base_id, /geom)
	xlc_orbit_camera.xpad = frame_info.xsize - width
	xlc_orbit_camera.ypad = frame_info.ysize - height

	; register the widget with the xmanager
	XManager, "xlc_orbit", xlc_orbitbase, $			
			EVENT_HANDLER = "xlc_orbit_ev", $
			GROUP_LEADER = GROUP, $		
