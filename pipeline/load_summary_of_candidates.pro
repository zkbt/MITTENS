PRO load_summary_of_candidates, summary_of_candidates
	...
	common mearth_tools
	common this_star
	f = file_search('ls*/combined/'+typical_candidate_filename)
...
; 	endif
	ls = long(stregex(/ext, stregex(/ext, f, 'ls[0-9]+'), '[0-9]+'))
	n = n_elements(f)
	template = create_struct('LS', 0l, 'STAR_DIR', '', {period:0.0d, hjd0:0.0d, duration:0.0, depth:0.0, depth_uncertainty:0.0, ratio:0.0, n_boxes:0, n_points:0, rescaling:1.0, inflation_for_bad_duration:1.0, ignore:0B, known:0B, variability:0B, stats:{boxes:fltarr(9), points:0, points_per_box:fltarr(9), start:0.0d, finish:0.0d, periods_searched:0L}} )
	...
	for i=0, n-1 do begin
		...
		if file_test(star_dir + 'box_pdf.idl') then begin
			if ~is_uptodate( star_dir + 'octopus_stat_summary.idl', f[i]) then begin
	...
	summary_of_candidates = summary_of_candidates[where(has_boxes and summary_of_candidates.depth_uncertainty gt 0)]
