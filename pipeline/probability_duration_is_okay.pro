FUNCTION probability_duration_is_okay, period_grid, duration_grid, shhh=shhh
	...
	if ~keyword_set(shhh) then begin
		...
		mprint, tab_string + tab_string + tab_string, 'assuming stellar (mass, radius) = (', string(form='(F4.2)', mo_info.mass),',', string(form='(F4.2)',mo_info.radius), ') solar' 
	endif
	tmax = period_grid/!pi*asin(1.0/a_over_rs(mo_info.mass, mo_info.radius, period_grid))
	width = 0.3*tmax
