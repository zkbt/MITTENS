	...
	for i=0, n_elements(input)-1 do begin
		...
		if typename(input) eq 'STRING' then begin
			...
			if strlen(attempted_mo) eq 16 then begin 
				mo[i] = attempted_mo
				;print, tab_string, 'interpreting ', attempted_mo, ' as a valid 2MASS MEarth identifier'
			endif else begin
				imatch = where(strmatch(mo_ensemble.bestname, this, /fold_case), nmatch)
				if nmatch eq 1 then begin
				  mo[i] = mo_ensemble[imatch].mo
				endif else begin
				  print, tab_string, 'name2mo.pro could not find a unique match for ', this
				  mo[i] = "???"
				endelse
			endelse
		endif
		if typename(this) eq 'INT' or typename(this) eq 'LONG' or typename(this) eq 'FLOAT' or typename(this) eq 'DOUBLE' then begin
			;print, tab_string, 'interpreting ', rw(this), ' as an old-school LSPM number'
			i_match = where(mo_ensemble.lspmn eq long(this), n_match)
			if n_match eq 0 then begin
				print, tab_string, error_string, 'but no matching MEarth Objects were found!'
				mo[i] = '???'
			endif else begin
				if n_match gt 1 then begin 
					print, tab_string, error_string, 'multiple matching MEarth Objects were found, using the first one!'
					print, tab_string, tab_string, mo_ensemble[i_match].mo
					mo[i] = mo_ensemble[i_match[0]].mo
				endif else begin
					mo[i] = mo_ensemble[i_match].mo
					;print, tab_string, tab_string, this, ' --> ', mo[i]
				endelse
			endelse
		endif
	endfor
