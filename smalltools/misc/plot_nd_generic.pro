PRO plot_nd_generic, clouds, eps=eps, dye=dye, res=res, tags=tags, symsize=symsize, psym=psym, flagged=flagged, charsize=charsize, log=log, fit=fit, fried_egg=fried_egg, multiple=multiple, names = names
	...
	;  by zach
	;
	;	where "c" is a structure containing "n" tags (either as c[*].tag or c.tag[*]), plot_nd with produce
	...
	if not keyword_set(dye) then  dye=1 
	if not keyword_set(res) then res = 20
	if not keyword_set(names) then names = tag_names(c)
	if keyword_set(tags) then names = names[tags] else tags=indgen(n_elements(names))
...
;	s = sort(c.p)
	...
	;			print, 'using ad hoc xrange', names[j]
		...
		;		print, 'using ad hoc xrange', names[i]
				...
				;	if i eq d-1 then xtitle=names[i] else xtitle=''
					...
					plothist, /nan, [xr[0],c.(tags[i])],   xstyle=1, ystyle=1, bin=(xr[0] - xr[1])/res,  xrange=xr, thick=3, xlog=log, ytickn=[' ',' ',' ',' ',' ',' ',' ',' ',' ',' '], xtickname=xtickname
					plothist, /nan, [xr[0],c.(tags[i])],   xstyle=1, ystyle=1, bin=(xr[0] - xr[1])/res,  xrange=xr, thick=3, xlog=log, title=names[i],  charsize=1.5*!p.charsize, charthick=1.5, ytickname=replicate('  ',20), xtickname=replicate('  ',20), xtitle=xtitle
				;	axis,  /xaxis
				;	axis, /ystyle, /yaxis
	; HACK TO HANDLE FLAT ARRAYS! DEAL WITH!
			;		if keyword_set(multiple) then for k=1, n_clouds-1 do plothist, /nan, /overplot, bin=(xr[0] - xr[1])/res,  thick=3, color=cloud_colors[k], [0, clouds.(k).(tags[i])], peak=1.0
					...
					if keyword_set(xtitle) then plot,  c.(tags[j]), c.(tags[i]), xstyle=1, xrange=xr, yrange=yr, ystyle=1,/nodata, ylog=log, xlog=log, xtitle=xtitle, charsize=1.5*!p.charsize, xtickname=replicate('  ',20), ytickname=replicate('  ',20),charthick=1.5
					if keyword_set(ytitle) then plot, c.(tags[j]), c.(tags[i]),  xstyle=1, xrange=xr, yrange=yr, ystyle=1,/nodata, ylog=log, xlog=log , ytitle=ytitle, charsize=1.5*!p.charsize, ytickname=replicate('  ',20), xtickname=replicate('  ',20),charthick=1.5

				;		oplot, [-1000,1000], [0,0], linestyle=1
				;		oplot, [0,0], [-1000,1000], linestyle=1

						if keyword_set(fried_egg) then begin
							if keyword_set(multiple) then for k=0, n_clouds-1 do fried_egg, clouds.(k).(tags[j]), clouds.(k).(tags[i]), /overplot, res=res, color=cloud_colors[k], xrange=xr, yrange=yr else fried_egg, c.(tags[j]), c.(tags[i]), /overplot, res=res , xrange=xr, yrange=yr 
						endif else begin
							if keyword_set(multiple) then for k=0, n_clouds-1 do plots, clouds.(k).(tags[j]), clouds.(k).(tags[i]), psym=psym, symsize=symsize, color=cloud_colors[k], noclip=0 else plots, c.(tags[j]), c.(tags[i]), psym=psym, color=(dye - min(dye))*254.0/max((dye - min(dye))), symsize=symsize, noclip=0
						endelse
