...
PRO smultiplot, pmulti, help=help, $
	...
	$
	rowheights=rowheights_in, colwidths=colwidths_in
    ...
    if init or keyword_set(initialize) then begin
        ...
        endcase
	if keyword_set(rowheights_in) then rowheights = [0.0,rowheights_in/total(rowheights_in)*nplots[1]] else rowheights=[0,fltarr(nplots[1]) + 1.0]
  if keyword_set(colwidths_in) then colwidths = [0.0,colwidths_in/total(colwidths_in)*nplots[0]] else colwidths=[0,fltarr(nplots[0]) + 1.0]
...
;   
    ...
    pos = $
        [(total(colwidths[0:col-1]))*idx[0], (rows - total(rowheights[0:row]))*idx[1],   $
         (total(colwidths[0:col]))*idx[0],     (rows - total(rowheights[0:row-1]))*idx[1]] $
       +                                      $
        [margins[0], margins[1],              $
         margins[0], margins[1]]
